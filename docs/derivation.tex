...
Each observation is the true calibration value plus independent Gaussian noise.
In the simplest (homoscedastic) case the noise has constant variance:
...
In many assays---particularly immunoassays and serial dilution experiments---the
measurement variance is not constant but increases with the signal level.
Following \citet{gelman2004}, the application offers an optional
heteroscedastic variance model that replaces the constant-variance assumption
(\Cref{ass:noise}) with a power-law relationship between the variance and the
predicted mean.

\subsection{Variance Model}

Let $\mu_i = f(x_i;\,\thetab)$ denote the predicted mean response at
observation~$i$.  The heteroscedastic model is
\begin{equation}\label{eq:hetero}
    \boxed{\;
    y_i \;\sim\; \N\!\biggl[\,\mu_i,\;\;
        \left(\frac{\mu_i}{A}\right)^{\!2\alpha}\!\sigma_y^{2}\;\biggr],
    \qquad i = 1,\dots,n,
    \;}
\end{equation}
where:
\begin{itemize}[nosep]
    \item $A$ is a scaling constant set to the geometric mean of the observed
          calibration responses,
          $A = \exp\!\bigl(\tfrac{1}{n}\sum_{i=1}^{n}\log y_i\bigr)$.
          Its role is to make $\sigma_y$ interpretable as the noise standard
          deviation at a ``typical'' measurement level.
    \item $\alpha \ge 0$ controls how variance scales with the mean:
    \begin{itemize}[nosep]
        \item $\alpha = 0$: constant variance (reduces to the homoscedastic
              model).
        \item $\alpha = 1$: variance proportional to~$\mu_i^{2}$, i.e.\
              approximately constant coefficient of variation (CV).
        \item $\alpha \in (0,2)$: intermediate and super-proportional
              variance structures.
    \end{itemize}
\end{itemize}
...
\begin{equation}\label{eq:sd-hetero}
    \sigma_i \;=\; \left|\frac{\mu_i}{A}\right|^{\!\alpha}\,\sigma_y\,,
\end{equation}
and the heteroscedastic likelihood becomes
\begin{equation}\label{eq:lik-hetero}
    p(\ycal \mid \xcal,\,\thetab,\,\sigma_y,\,\alpha)
    \;=\; \prod_{i=1}^{n}
        \frac{1}{\sqrt{2\pi}\,\sigma_i}\,
        \exp\!\biggl[-\frac{(y_i - \mu_i)^{2}}{2\,\sigma_i^{2}}\biggr].
\end{equation}

\subsection{Prior on \texorpdfstring{$\alpha$}{alpha}}

We assign $\alpha$ a uniform prior on $[0,2]$:
\begin{equation}\label{eq:prior-alpha}
    \alpha \;\sim\; \mathrm{Uniform}(0,\,2).
\end{equation}
This range is centred at proportionality ($\alpha=1$) and is wide enough to
accommodate the constant-variance special case ($\alpha\approx 0$) as well as
super-proportional variance.  The data determine the posterior for~$\alpha$;
when constant variance is adequate, the posterior will concentrate near zero.
...
The application allows the user to choose prior distributions for all
...
Normal, Half-Normal, Uniform, or Log-Normal distributions, each with
...
\begin{equation}\label{eq:joint-prior}
    p(\thetab,\,\sigma_y,\,\alpha)
    \;=\;
    \biggl[\prod_{j=1}^{p} p(\theta_j)\biggr]\;p(\sigma_y)\;p(\alpha).
\end{equation}
(When the homoscedastic model is used, the $p(\alpha)$ factor is absent and
$\alpha$ is not part of the model.)
...
\begin{equation}\label{eq:bayes}
    \boxed{\;
    p(\thetab,\,\sigma_y,\,\alpha \mid \ycal,\,\xcal)
    \;=\;
    \frac{p(\ycal \mid \xcal,\,\thetab,\,\sigma_y,\,\alpha)\;\;
          p(\thetab,\,\sigma_y,\,\alpha)}
         {p(\ycal \mid \xcal)}
    \;}
\end{equation}
...
\begin{equation}
    p(\ycal \mid \xcal)
    \;=\;
    \int p(\ycal \mid \xcal,\,\thetab,\,\sigma_y,\,\alpha)\;
         p(\thetab,\,\sigma_y,\,\alpha)
    \;\mathrm{d}\thetab\;\mathrm{d}\sigma_y\;\mathrm{d}\alpha.
\end{equation}
...
sampling.  (When the homoscedastic model is used, $\alpha$ is absent from
all expressions.)
...
\paragraph{Heteroscedastic case.}
The noise standard deviation at the new point depends on the predicted mean
$\mu^{*} = f(x^{*};\,\thetab)$, which is unknown.  We approximate it by using
the observed value~$y^{*}$ as a plug-in for~$\mu^{*}$:
\begin{equation}\label{eq:new-obs-hetero}
    \varepsilon^{*} \;\sim\; \N\!\Bigl(0,\;\;
        \bigl|y^{*}/A\bigr|^{2\alpha}\,\sigma_y^{2}\Bigr).
\end{equation}
This approximation is accurate when $y^{*}$ is close to~$\mu^{*}$ (i.e.\ when
the signal-to-noise ratio is moderate to high).
...
\begin{equation}\label{eq:ppd}
    ...
    p(x^{*} \mid y^{*},\,\ycal,\,\xcal)
    \;=\;
    \int p(x^{*} \mid y^{*},\,\thetab,\,\sigma_y,\,\alpha)\;\;
         p(\thetab,\,\sigma_y,\,\alpha \mid \ycal,\,\xcal)
    \;\mathrm{d}\thetab\;\mathrm{d}\sigma_y\;\mathrm{d}\alpha
    \;}
\end{equation}
...
For each posterior draw $(\thetab^{(s)},\,\sigma_y^{(s)},\,\alpha^{(s)})$,
$s=1,\dots,N$:
\begin{enumerate}[nosep]
    \item \textbf{Compute noise scale:}\;
          $\sigma^{*(s)} = \bigl|y^{*}/A\bigr|^{\alpha^{(s)}}\,\sigma_y^{(s)}$
          \;\;(or simply $\sigma^{*(s)} = \sigma_y^{(s)}$ in the homoscedastic
          case).
...
\begin{proof}
For fixed $(\thetab,\sigma_y,\alpha)$ the forward model is deterministic, so
...
Since the draws $(\thetab^{(s)},\sigma_y^{(s)},\alpha^{(s)})$ come from the
posterior $p(\thetab,\sigma_y,\alpha\mid\ycal,\xcal)$, the composition produces
samples from the marginal~\eqref{eq:ppd}.
\end{proof}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Fan-shaped spread:} enable the \textbf{heteroscedastic
          variance model} in Advanced Options, which lets the model learn how
          noise scales with the mean response.  Alternatively, try a
          variance-stabilising transformation such as $\log(y)$ or $\sqrt{y}$
          before fitting.
...
\begin{enumerate}[nosep,leftmargin=2em]
    \item The user-specified function $f(x;\thetab)$ adequately describes the
          ...
          (\Cref{ass:func}).
    \item Observation noise is additive and Gaussian, with either constant
          variance~$\sigma_y^{2}$ or signal-dependent variance
          $(\mu_i/A)^{2\alpha}\sigma_y^{2}$
...
\begin{itemize}[nosep]
    ...
    \item \emph{Non-constant variance.}  The Breusch--Pagan test will flag
          this; enable the heteroscedastic variance model or apply a
          variance-stabilising transform (e.g.\ $\log y$) and re-fit.
