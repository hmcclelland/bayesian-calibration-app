...
Each observation equals the true mean response plus independent Gaussian noise.
...
\subsection{Built-in Variance Functions}

The application provides three preset choices and a free-form custom entry:

\begin{center}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{lll}
    \toprule
    Name & $g(\mu_i,\,\sigma_y;\,\phib)$ & Notes \\
    \midrule
    Constant & $\sigma_y$ & Homoscedastic; $\phib = \varnothing$ \\[2pt]
    Proportional to mean & $\mu_i\,\sigma_y$ & Constant coefficient of variation (CV) \\[2pt]
    Power-law & $\bigl|\mu_i/A\bigr|^{\gamma}\,\sigma_y$
        & $A$: geometric mean of $\ycal$;\; $\phib = \{\gamma\}$ \\
    \bottomrule
\end{tabular}
\end{center}

\noindent
In the power-law model, $\gamma \ge 0$ controls how the noise scales with the
mean ($\gamma=0$: constant variance; $\gamma=1$: constant CV; $\gamma>1$:
super-proportional variance), and $A =
\exp\!\bigl(\tfrac{1}{n}\sum_{i=1}^{n}\log y_i\bigr)$ is a fixed scaling
constant that makes $\sigma_y$ interpretable as the noise standard deviation at
a typical measurement level.  \citet{gelman2004} used this power-law form to
model serial dilution immunoassay data; it subsumes the constant and
proportional presets as special cases.  For the power-law model we assign
$\gamma$ a uniform prior on $[0,2]$:
\begin{equation}\label{eq:prior-gamma}
    \gamma \;\sim\; \mathrm{Uniform}(0,\,2).
\end{equation}
More generally, priors on~$\phib$ are specified by the user in the same way as
priors on~$\thetab$ (see \Cref{sec:priors}).
...
\begin{equation}\label{eq:joint-prior}
    ...
    \;=\;
    \biggl[\prod_{j=1}^{p} p(\theta_j)\biggr]\;p(\sigma_y)\;p(\phib).
\end{equation}
When the constant-variance model is used $\phib$ is empty and the last
factor is absent.
...
\begin{equation}
    ...
    \mu^{*} = f(x^{*};\,\thetab).
\end{equation}
Because $x^{*}$ is unknown, $\mu^{*}$ is also unknown.  We approximate it by
substituting the observed value~$y^{*}$ as a plug-in for~$\mu^{*}$:
...
\begin{proof}
Fix a posterior draw $(\thetab,\sigma_y,\phib)$.  By the observation
model~\eqref{eq:new-obs}, the unknown satisfies
$x^{*} = f^{-1}(y^{*} - \varepsilon^{*};\,\thetab)$ where
$\varepsilon^{*}\sim\N(0,\sigma^{*2})$.  Because the Gaussian distribution is
symmetric, $-\varepsilon^{*}\overset{d}{=}\varepsilon^{*}$, so drawing
$\tilde{y}^{(s)} = y^{*} + \epsilon^{(s)}$ with
$\epsilon^{(s)}\sim\N(0,\sigma^{*(s)2})$ and setting
$x^{*(s)} = f^{-1}(\tilde{y}^{(s)};\,\thetab^{(s)})$ is equivalent in
distribution to sampling $x^{*} = f^{-1}(y^{*} - \varepsilon^{*};\,\thetab)$.
Since the draws come from the posterior
$p(\thetab,\sigma_y,\phib\mid\ycal,\xcal)$, the composition produces
...
\begin{itemize}[nosep]
    \item \textbf{Curved or trending residuals:} try a different model (e.g.\
          add a polynomial term, switch to an exponential or sigmoid function).
          The chosen functional form does not adequately capture the
    ...
    \item \textbf{Significant Breusch--Pagan test ($p < 0.05$) on standardised
          ...
          heteroscedasticity; consider a different variance equation.
    \item \textbf{Random scatter around zero with non-significant test:} the
          model assumptions appear satisfied and inverse predictions can be
          trusted.
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item \emph{Non-Gaussian noise.}  Heavy tails may cause credible intervals
          ...
          work.
    \item \emph{Correlated observations.}  If measurements are
          time-dependent or spatially structured, the independence assumption
          is violated and intervals will be too narrow.
\end{itemize}
